...
// 外部アドオン: API を呼び出す
await router.request("werewolf-gamemanager", "game:killPlayer", { playerId });
```
...
```typescript
// 騎士アドオン（startup 内）: game:killPlayer の前に割り込む
...
router.emit("game:playerDie", { playerId, roleId });

// 外部アドオン: startup 内で購読を宣言
ev.addonEvents.on("werewolf-gamemanager", "game:playerDie", (payload) => { ... });
```
